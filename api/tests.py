...
class TicketListCreateTestCase(APITestCase):
    ...
    def test_create_ticket_successfully(self):
        ...
        resp = self.client.post(reverse('api:ticket-list'), data, **self.headers, format='json')
    ...
    def test_ticket_listing(self):
        Ticket.objects.create(title='Test title', description='Test desc', reporter=self.user, type='minor',
                              status='in progress')
        resp = self.client.get(reverse('api:ticket-list'), {}, **self.headers, format='json')
...
class TicketRetrieveUpdateDeleteTestCase(APITestCase):
    ...
    def test_retrieve_ticket(self):
        resp = self.client.get(reverse('api:ticket-detail', args=[self.ticket.id]), {}, **self.headers, format='json')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
    ...
    def test_update_ticket(self):
        ...
        resp = self.client.put(reverse('api:ticket-detail', args=[self.ticket.id]), data, **self.headers, format='json')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(resp.data['title'], 'Test title updated')
        self.assertEqual(resp.data['type'], 'blocker')

    def test_delete_ticket(self):
        resp = self.client.delete(reverse('api:ticket-detail', args=[self.ticket.id]), {}, **self.headers, format='json')
...
class UserTicketTestCase(APITestCase):
    ...
    def test_list_user_tickets(self):
        resp = self.client.get(reverse('api:user-tickets'), {}, **self.headers, format='json')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.data), 1)
...
class SearchTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        for data in data_list:
            self.client.post(reverse('api:ticket-list'), data, **self.headers, format='json')

    def test_search_with_results_in_tag(self):
        resp = self.client.post(reverse('api:search'), dict(search_text='tag1'), **self.headers, format='json')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.data), 4)
        self.assertEqual(resp.data[0]['description'], 'Test Description one')

    def test_search_with_no_results(self):
        resp = self.client.post(reverse('api:search'), dict(search_text='qwert'), **self.headers, format='json')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.data), 0)

    def test_search_with_results_in_type(self):
        resp = self.client.post(reverse('api:search'), dict(search_text='block'), **self.headers, format='json')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.data), 1)
        self.assertEqual(resp.data[0]['title'], 'Test title four')

    def test_search_with_results_in_tag_and_type(self):
        resp = self.client.post(reverse('api:search'), dict(search_text='crit'), **self.headers, format='json')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.data), 2)
        self.assertEqual(resp.data[0]['title'], 'Test title three')


class TicketsWithTagTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        data_list = [{"title": "Test title one", "description": "Test Description one",
                      "type": "major", "status": "backlog", "tags": [{"text": "tag10"}, {"text": "tag23"}]},
                     {"title": "Test title two", "description": "Test Description two",
                      "type": "minor", "status": "backlog", "tags": [{"text": "critical"}, {"text": "tag24"}]},
                     {"title": "Test title three", "description": "Test Description three",
                      "type": "critical", "status": "backlog", "tags": [{"text": "tag14"}, {"text": "tag25"}]},
                     {"title": "Test title four", "description": "Test Description four",
                      "type": "blocker", "status": "backlog", "tags": [{"text": "tag16"}, {"text": "critical"}]}]
    ...
    def test_get_all_tickets_with_tag(self):
        resp = self.client.get(reverse('api:tickets-with-tag', args=['critical']), {'tag': 'critical'}, **self.headers, format='json')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.data), 2)
...
class HomeTestCase(APITestCase):
    ...
    def test_home_page(self):
        resp = self.client.get(reverse('api:home'))
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
